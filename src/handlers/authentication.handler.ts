...
import { CreateTicketDTO } from '../ticket/models/dto/create-ticket.dto.js';
...
export class AuthenticationHandler implements KafkaEventHandler {
  ...
  /**
   * Creates a new instance of {@link AuthenticationHandler}.
   *
   ...
   */
  @KafkaEvent(...getTopics('createTicket'))
  async handle(
    topic: string,
    data: { payload: CreateTicketDTO },
    ...
    switch (topic) {
      case getTopic('createTicket'):
        await this.createTickets(data as { payload: CreateTicketDTO });
  ...
  private async createTickets(data: {
    ...
    await this.ticketWriteService.createTicket(data.payload);
  }
}
